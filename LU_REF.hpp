...
#define _ASSUME(X) return X;

#define MaskOf(X) _blsi_u64(X)
#define SquareOf(X) _tzcnt_u64(X)
#define Bitloop(X) for(;X; X = _blsr_u64(X))

_Compiletime uint64_t GetSliderHCond(uint64_t SliderSquare, uint64_t occupy) {
	uint64_t result{};
	switch (SliderSquare) {
	case 0:
		result = 2ull;
...
_Compiletime uint64_t GetSliderVCond(uint64_t SliderSquare, uint64_t occupy) {
	uint64_t result{};
	switch (SliderSquare) {
	case 0:
		result = 256ull;
...
_Compiletime uint64_t GetSliderD2Cond(uint64_t SliderSquare, uint64_t occupy) {
	uint64_t result{};
	switch (SliderSquare) {
	case 0:
		result = 0ull;
...
_Compiletime uint64_t GetSliderD1Cond(uint64_t SliderSquare, uint64_t occupy) {
	uint64_t result{};
	switch (SliderSquare) {
	case 0:
		result = 512ull;
...
namespace Lookup_Ref {
	...
	_Compiletime uint64_t rookAttacks(int sq, uint64_t occ) {
		return GetSliderHCond(sq, occ) | GetSliderVCond(sq, occ);
	}

	_Compiletime uint64_t bishopAttacks(int sq, uint64_t occ) {
		return GetSliderD1Cond(sq, occ) | GetSliderD2Cond(sq, occ);
	}

	_Compiletime uint64_t rook_xray(int sq, uint64_t occ) {
		uint64_t attacks = rookAttacks(sq, occ);
		return attacks ^ rookAttacks(sq, occ ^ (occ & attacks));
	}

	_Compiletime uint64_t bishop_xray(int sq, uint64_t occ) {
		uint64_t attacks = bishopAttacks(sq, occ);
		return attacks ^ bishopAttacks(sq, occ ^ (occ & attacks));
	}

	_Compiletime uint64_t queenAttacks(int sq, uint64_t occ) {
